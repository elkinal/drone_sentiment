...
#TODO: apply this globally, to the entire dataset, then look at the most negative and most positive sentences. 

sentiment("hello there this is a cool sentence")$sentiment


get_sentiment <- function(x) {
  return (sentiment(x)$sentiment)
}
...
mini <- head(response, 5)
mini



#df <- data.frame(c("good","medium","bad"), c("ok","happy","win"))

#df <- mini$reason
#df

#df[] <- lapply(df, get_sentiment)
#df
#as.data.frame(t(df))
...
#vect = as.vector(head(response$reason, 10))
#vect

# turn column into vector, run sentiment on vector, turn vector into column, attach it...

#stm = lapply(vect, FUN=sentiment, simplify=TRUE)
#stm

#length(stm)

#com = rbind(stm[1], stm[2])
#com

#first = stm[1]

#for (x in 2:length(stm)) {
  #print(stm[x])
  #append(stms, stm[x])
#  first <- rbind(first, stm[x]$sentiment)
#} 

#stms
#first

#mini$sentiment <- sentiment(mini$reason)$sentiment 
```

```{r}
bk <- response
response <- head(response, 100)
...
#stm <- get_sentiment((head(response,1))$reason)
#stm 
#response |>
    #DELETEME
...
for (x in 1:nrow(response)) {
  rslt = get_sentiment((response$reason)[x])

  print(rslt)
  stm <- append(stm, list(rslt))
} 

df <- data.frame(unlist(stm)) 

df

new <- cbind(response, df)
new
...
statement <- as.vector(response$reason)

corpus <- tibble(document=1:nrow(response), text = statement)
#corpus
