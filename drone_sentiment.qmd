...
get_sentiment("Economic benefit") #4
...
#for (x in 1:nrow(df)) {
  #print(df$reason[x])
  #print(get_sentiment(df$reason[x]))
#  print(paste0(x, get_sentiment(df$reason[x])))
#} 

#vr = (get_sentiment(df$reason[31]))
print("------")
vr = sentiment(df$reason[775])
typeof(vr)
vr
response$reason[775]


get_sentiment("I do believe we should send any drone anywhere")
get_sentiment("Its a privacy violation")

# Basically the issue is that the sentence is split by punctuation. Remove all full stops to fix this.
```
...
score_min_max = rbind(head(score_responses, 10), tail(score_responses, 10))
score_min_max

grid.table(score_min_max)

# just format these as separate tibbles, print them out with an explanatory paragraph. Also mention that almost all of these responses are garbage.
```
